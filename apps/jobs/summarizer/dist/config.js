...
const DEFAULT_SYSTEM_PROMPT = "You are a helpful assistant that summarizes technical articles for software engineers. Generate a concise summary (<= 120 Japanese characters) highlighting the key takeaways.";
function loadConfig() {
    const projectId = process.env.GCP_PROJECT_ID ||
        process.env.GOOGLE_CLOUD_PROJECT ||
        process.env.GCLOUD_PROJECT;
    if (!projectId) {
        throw new Error("GCP project ID is required. Set GCP_PROJECT_ID or GOOGLE_CLOUD_PROJECT.");
    }
    const feeds = (process.env.RSS_FEEDS || "")
        ...
        .filter(Boolean);
    if (feeds.length === 0) {
        console.warn("[summarizer] RSS_FEEDS is empty. The job will exit without processing.");
    }
    ...
    if (!openAiKey) {
        throw new Error("OPENAI_API_KEY must be provided (use Secret Manager for production).");
    }
    const openAiModel = process.env.OPENAI_MODEL || "gpt-4o-mini";
    const systemPrompt = process.env.OPENAI_SYSTEM_PROMPT || DEFAULT_SYSTEM_PROMPT;
    const deeplKey = process.env.DEEPL_API_KEY;
    if (!deeplKey) {
        throw new Error("DEEPL_API_KEY must be provided (use Secret Manager for production).");
    }
...
function normalizeTargetLang(value) {
    ...
    const lookup = new Map(canonicalPairs);
